...
void ABlasterPlayerController::CheckPing(float DeltaSeconds)
{
	...
	if( HighPingRunningTime>CheckPingFrequency )
	{
		...
		if(PlayerState)
		{
			if(PlayerState->GetPing() * 4 >HighPingThreshold)//这其实是ping的四分之一，被压缩了
			{
				HighPingWarning();
	...
	if(CharacterOverlay==nullptr)
	{
		if(BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if(CharacterOverlay)
			{
				...
				if(bInitializeDefeats) SetHUDScore(HUDScore);
...
void ABlasterPlayerController::HandleCooldown()
{
	...
	if(BlasterHUD)
	{
		...
		if(bHUDValid)
		{
			...
			//BlasterHUD->Announcement->InfoText->SetVisibility(ESlateVisibility::Hidden);
